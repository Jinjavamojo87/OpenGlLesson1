...
{
    auto alfa = new osg::MatrixTransform;
...
// glutReshapeFunc
void OpenGLWrapper::resizeGL(int w, int h)
{
    glViewport(0, 0, w, h);
...
void OpenGLWrapper::scene()
{
    mainLoop();
    Display();
//    int id = QFontDatabase::addApplicationFont("/usr/share/fonts/truetype/liberation/LiberationSans-Regular.ttf");
//    QString family = QFontDatabase::applicationFontFamilies(id).at(0);
//    QFont monospace(family);
//    renderText(100,100,"OOOOOO",monospace);

    /*
    gARTImage = arVideoGetImage();
    if (gARTImage ==nullptr)
        return;
...
void OpenGLWrapper::timerEvent(QTimerEvent *)
{
    paintGL();
}
...
static void printText(const char *text, const float x, const float y, int calculateXFromRightEdge, int calculateYFromTopEdge)
{
    ...
    if (calculateXFromRightEdge) {
        x0 = prefWidth - x - (float)glutBitmapLength(GLUT_BITMAP_9_BY_15, (const unsigned char *)text);
    } else {
        x0 = x;
    }
    if (calculateYFromTopEdge) {
        y0 = prefHeight - y - 10.0f;
    } else {
        y0 = y;
    }
    glRasterPos2f(x0, y0);
...
static void changeDisplayInfoVisible(bool visible) {
    isDisplayObjectInfo = visible;
}
